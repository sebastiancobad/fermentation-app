...
8. Apply fouling (all resistances referred to outside tube surface area)
   1/U_o_clean = 1/U_assumed
...
─────────────────────────────
1. Ideal crossflow coefficient (h_id) — Taborek (1983):

   j_H = a1 × (1.33 / (p_t / d_o))^a × Re_s^a2

   Coefficients a1, a2, a are functions of:
   - Layout angle (30°, 45°, 60°, 90°)
   - Reynolds number range

   | Layout | Re range     | a1       | a2      | a      |
   |--------|-------------|----------|---------|--------|
   | 30°    | 10⁴ – 10⁵  | 0.321    | -0.388  | 1.450  |
   | 30°    | 10³ – 10⁴  | 0.321    | -0.388  | 1.450  |
   | 30°    | 10² – 10³  | 0.593    | -0.477  | 1.450  |
...
2. Heat transfer:
   Turbulent (Gnielinski, Re > 4000):
     Nu = (f/8)(Re - 1000)Pr / [1 + 12.7(f/8)^0.5(Pr^(2/3) - 1)]
     h_tube = Nu × k / d_i

   Transition zone (2300 < Re < 4000):
     Linear interpolation between laminar and turbulent:
     h_transition = h_laminar + (h_turbulent - h_laminar) × (Re - 2300) / (4000 - 2300)

   Laminar (Sieder-Tate, Re < 2300):
     Nu = 1.86 × (Re × Pr × d_i/L)^(1/3) × (μ/μ_w)^0.14

   Note: The transition zone interpolation avoids discontinuities in h(Re)
...
1. Overall coefficient:
   1/U_o_clean = 1/h_o + (d_o × ln(d_o/d_i))/(2 × k_wall) + (d_o/d_i)/h_i
   1/U_o_dirty = 1/U_o_clean + R_f_o + (d_o/d_i) × R_f_i

   Where:
   h_o = h_shell (corrected Bell-Delaware)
   h_i = h_tube (Gnielinski, Sieder-Tate, or transition interpolation)
   R_f_o = shell-side (outside) fouling resistance [m²·K/W]
   R_f_i = tube-side (inside) fouling resistance [m²·K/W]
   k_wall = tube wall thermal conductivity [W/(m·K)]

   Note: The tube wall resistance term (d_o × ln(d_o/d_i))/(2×k_wall) is often
   negligible for carbon steel/copper tubes but significant for titanium or
   polymer-lined tubes.
