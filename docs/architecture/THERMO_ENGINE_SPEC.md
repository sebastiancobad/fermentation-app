...
3. Select correct root by minimizing Gibbs energy: G = RT[Z - 1 - ln(Z - B) - A/(2√2·B) × ln((Z + (1+√2)B)/(Z + (1-√2)B))]

### 3.3 Derived Properties from EOS

```
Fugacity coefficient:
  ln(φ_i) = (b_i/b_mix)(Z-1) - ln(Z-B) - A/(2√2·B) × (2Σ(x_j·a_ij)/a_mix - b_i/b_mix) × ln((Z+(1+√2)B)/(Z+(1-√2)B))

Enthalpy departure:
  H - H_ig = RT(Z-1) + [T(da/dT) - a] / (2√2·b) × ln((Z+(1+√2)B)/(Z+(1-√2)B))

Entropy departure:
  S - S_ig = R×ln(Z-B) + (da/dT)/(2√2·b) × ln((Z+(1+√2)B)/(Z+(1-√2)B))
...
```json
{
  ...
  "bubble_curve": [
    { "temperature": -161.5, "pressure": 1.013, "unit_T": "°C", "unit_P": "bar" },
    { "temperature": -140.0, "pressure": 5.2, "unit_T": "°C", "unit_P": "bar" },
    "...100 points..."
  ],
  "dew_curve": [
    { "temperature": -161.5, "pressure": 1.013, "unit_T": "°C", "unit_P": "bar" },
    { "temperature": -120.0, "pressure": 8.5, "unit_T": "°C", "unit_P": "bar" },
    "...100 points..."
  ],
  "critical_point": {
