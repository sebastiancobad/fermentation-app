...
CREATE UNIQUE INDEX idx_correlations_unique
    ON property_correlations(component_id, property_name, equation_number);
...
CREATE TABLE binary_interaction_params (
    ...
    -- Ensure component_1_id < component_2_id to avoid duplicates
    CONSTRAINT chk_component_order CHECK (component_1_id < component_2_id),
...
BEGIN
    CASE p_equation_number
        ...
        WHEN 106 THEN
            -- Y = A * (1-Tr)^(B + C*Tr + D*Tr^2)  [Needs Tc passed as p_e]
            DECLARE v_tr DOUBLE PRECISION := p_t / p_e;
            BEGIN
                RETURN p_a * (1.0 - v_tr)^(p_b + p_c*v_tr + COALESCE(p_d,0)*v_tr^2);
            END;
        WHEN 107 THEN
            -- Y = A + B*((C/T)/sinh(C/T))^2 + D*((E/T)/cosh(E/T))^2
            RETURN p_a + p_b*((p_c/p_t)/sinh(p_c/p_t))^2 + COALESCE(p_d,0)*((COALESCE(p_e,0)/p_t)/cosh(COALESCE(p_e,0)/p_t))^2;
        ELSE
